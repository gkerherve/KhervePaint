...
def test_chem_bonds_rings_atoms(scene):
    from khervepaint.canvas import (PathItem, PolygonItem, EllipseItem,
...
def test_chem_bond_roundtrips_svg(scene, tmp_path):
    from khervepaint import svgio
    from khervepaint.canvas import PathItem, CHEM_DOUBLE
...
def test_chem_chain_connects_bonds(scene):
    from khervepaint.canvas import LineItem, CHEM_CHAIN
...
def test_chem_atom_snaps_to_bond_end(scene):
    from khervepaint.canvas import LineItem, TextItem
    scene.dpi = 300
    scene.snap_enabled = True
...
def test_floorplan_all_elements_build(app):
    from khervepaint import floorplan
...
def test_science_symbol_modules_build(app):
    ...
    from khervepaint import (optics, vacuum, labware, flowchart,
...
def test_text_anchor_centering(app):
    ...
    from khervepaint.ai_assistant import _spec_to_item
...
def test_valve_triangles_meet_at_seat(app):
    ...
    from khervepaint import vacuum, pid
    from khervepaint.ai_assistant import _spec_to_item
    cx = cy = 50.0
...
def test_electrical_all_elements_build_and_place(scene):
    from khervepaint import electrical
...
def test_room_tool_draws_wall_rect(scene):
    from khervepaint.canvas import ROOM
    ...
    rects = [c for c in items[0].childItems() if isinstance(c, RectItem)]
    assert len(rects) == 8                              # 4 wall bars + 4 corners
    filled = [r for r in rects if r.brush().style() != Qt.NoBrush]
    empty = [r for r in rects if r.brush().style() == Qt.NoBrush]
    assert len(filled) == 4 and len(empty) == 4         # solid walls, empty corners
    br = items[0].sceneBoundingRect()                    # ~drag size (+pen width)
    assert abs(br.width() - 200) <= 3 and abs(br.height() - 150) <= 3
...
def test_chem_hydrogen_bond_dashes_roundtrip(scene, tmp_path):
    ...
    from khervepaint import svgio
    from khervepaint.canvas import CHEM_HBOND
...
def test_open_path_loads_svg_for_khervebook_handoff(window, tmp_path):
    """KherveBook writes an SVG and launches ``khervepaint <file>``; the
