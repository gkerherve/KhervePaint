...
class ColorButton(QToolButton):
    ...
    def _pick(self):
        c = QColorDialog.getColor(self._color, self, self.tr("Choose colour"),
...
class PropertiesDialog(QDialog):
    ...
    def __init__(self, item, parent=None):
        super().__init__(parent)
        self.item = item
        self.setWindowTitle(self.tr("Item properties"))
    ...
    def _build_common(self, layout):
        form = self._section(layout, self.tr("Transform"))
        ...
        form.addRow(self.tr("Rotation"), self.rotation)
        form.addRow(self.tr("Opacity"), self.opacity)

    def _build_stroke(self, layout):
        form = self._section(layout, self.tr("Stroke"))
        pen = self.item.pen()
        self.stroke_on = QCheckBox(self.tr("Stroke"))
        ...
        form.addRow(self.tr("Colour"), self.stroke_color)
        form.addRow(self.tr("Width"), self.stroke_width)

    def _build_fill(self, layout):
        form = self._section(layout, self.tr("Fill"))
        brush = self.item.brush()
        spec = gradient.brush_spec(brush)
        self.fill_on = QCheckBox(self.tr("Fill"))
        ...
        self.fill_style.addItems([self.tr("Solid"), self.tr("Linear gradient"),
                                  self.tr("Radial gradient"),
                                  self.tr("Sun (lit from top-left)")])
        ...
        form.addRow(self.tr("Style"), self.fill_style)
        form.addRow(self.tr("Colour"), self.fill_color)
        form.addRow(self.tr("End colour"), self.fill_color2)
        form.addRow(self.tr("Angle °"), self.fill_angle)

    def _build_geometry(self, layout):
        item = self.item
        if isinstance(item, LineItem):           # line + arrow
            form = self._section(layout, self.tr("Endpoints"))
            ln = item.line()
            self.x1 = _spin(ln.x1()); self.y1 = _spin(ln.y1())
            self.x2 = _spin(ln.x2()); self.y2 = _spin(ln.y2())
            form.addRow(self.tr("X1"), self.x1); form.addRow(self.tr("Y1"), self.y1)
            form.addRow(self.tr("X2"), self.x2); form.addRow(self.tr("Y2"), self.y2)
            if not isinstance(item, DimensionItem):
                bend = item.bend()
                mid = (ln.p1() + ln.p2()) / 2
                self.curved = QCheckBox(self.tr("Curved (bend through a control "
                                        "point)"))
                ...
                form.addRow(self.tr("Bend X"), self.bend_x)
                form.addRow(self.tr("Bend Y"), self.bend_y)
        elif isinstance(item, (RectItem, EllipseItem, RoundedRectItem)):
            form = self._section(layout, self.tr("Geometry"))
            r = item.rect()
            self.rx = _spin(r.x()); self.ry = _spin(r.y())
            self.rw = _spin(r.width(), 0); self.rh = _spin(r.height(), 0)
            form.addRow(self.tr("X"), self.rx); form.addRow(self.tr("Y"), self.ry)
            form.addRow(self.tr("Width"), self.rw); form.addRow(self.tr("Height"), self.rh)
            if isinstance(item, RoundedRectItem):
                self.radius = _spin(item.radius(), 0, 1000, 1)
                form.addRow(self.tr("Corner radius"), self.radius)
        elif isinstance(item, PolygonItem):
            form = self._section(layout, self.tr("Geometry"))
            form.addRow(QLabel(self.tr("{0} vertices ({1})").format(
                item.polygon().count(), item.kind)))

    def _build_dimension(self, layout):
        form = self._section(layout, self.tr("Dimension style"))
        item = self.item
        self.dim_cap = QComboBox()
        self.dim_cap.addItems([self.tr("arrows"), self.tr("ticks"),
                               self.tr("dots"), self.tr("none")])
        self.dim_cap.setCurrentText(item.cap_style)
        self.dim_ext = QCheckBox(self.tr("Extension (witness) lines"))
        self.dim_ext.setChecked(item.extension)
        self.dim_dash = QCheckBox(self.tr("Dashed line"))
        ...
        self.dim_prefix.setPlaceholderText(self.tr("e.g. Ø, ±"))
        self.dim_suffix = QLineEdit(item.suffix)
        form.addRow(self.tr("End caps"), self.dim_cap)
        form.addRow(self.dim_ext)
        form.addRow(self.dim_dash)
        form.addRow(self.tr("Unit"), self.dim_unit)
        form.addRow(self.tr("Decimals"), self.dim_decimals)
        form.addRow(self.tr("Prefix"), self.dim_prefix)
        form.addRow(self.tr("Suffix"), self.dim_suffix)

    def _build_text(self, layout):
        form = self._section(layout, self.tr("Text"))
    ...
    def _build_label(self, layout):
        ...
        self.label_bold = QCheckBox(self.tr("Bold"))
        self.label_bold.setChecked(self.item._label_bold)
        self.label_italic = QCheckBox(self.tr("Italic"))
        self.label_italic.setChecked(self.item._label_italic)
        self.label_color = ColorButton(self.item.label_color())
        form.addRow(self.tr("Text"), self.label_text)
        form.addRow(self.tr("Font"), self.label_family)
        form.addRow(self.tr("Size"), self.label_size)
        form.addRow(self.label_bold)
        form.addRow(self.label_italic)
        form.addRow(self.tr("Colour"), self.label_color)
...
def stack_cells_dialog(window, item):
    ...
    dlg = QDialog(window)
    dlg.setWindowTitle(QCoreApplication.translate("PropertiesDialog", 'Stack unit cells'))
    layout = QVBoxLayout(dlg)
    layout.addWidget(QLabel(QCoreApplication.translate("PropertiesDialog", 'Repeat the unit cell along each lattice vector:')))
    form = QFormLayout()
    spins = []
...
def build_context_menu(window, item) -> QMenu:
    ...
    menu = QMenu(window)
    menu.addAction(icons.icon("mdi.pencil-box-outline"), QCoreApplication.translate("PropertiesDialog", 'Edit properties…'),
                   lambda: window.edit_item(item))
    if getattr(item, "rxn_data", None):        # reaction scheme
        from .reactionview import open_reaction_builder
        menu.addAction(icons.icon("mdi.flask-outline"), QCoreApplication.translate("PropertiesDialog", 'Edit reaction…'),
    ...
    if getattr(item, "mol_name", None):        # molecule / crystal model
        menu.addAction(icons.icon("mdi.molecule"), QCoreApplication.translate("PropertiesDialog", 'Molecule builder…'),
                       lambda: window.view.open_molecule_builder(item))
        menu.addAction(icons.icon("mdi.rotate-3d-variant"), QCoreApplication.translate("PropertiesDialog", 'Rotate in 3D'),
                       lambda: window.scene.enter_orbit_mode(item))
        from . import molrepr, molecules
        show_as = menu.addMenu(icons.icon("mdi.eye-outline"), QCoreApplication.translate("PropertiesDialog", 'Show as'))
        ...
        if molecules.can_stack(item.mol_name):
            menu.addAction(icons.icon("mdi.cube-outline"), QCoreApplication.translate("PropertiesDialog", 'Stack unit cells…'),
                           lambda: stack_cells_dialog(window, item))
        if molecules.has_polyhedra(item.mol_name):
            act = menu.addAction(icons.icon("mdi.pyramid"),
                                 QCoreApplication.translate("PropertiesDialog", 'Show polyhedra'))
        ...
        menu.addAction(icons.icon("mdi.palette-outline"), QCoreApplication.translate("PropertiesDialog", 'Add colour legend'),
                       lambda: molcolor.place_legend(window.scene, item))
    if isinstance(item, ImageItem):
        menu.addAction(icons.icon("mdi.crop"), QCoreApplication.translate("PropertiesDialog", 'Crop image'),
                       lambda: window.crop_image(item))
        menu.addAction(icons.icon("mdi.image-remove"), QCoreApplication.translate("PropertiesDialog", 'Remove background'),
                       lambda: window.remove_image_background(item))
    menu.addSeparator()
    menu.addAction(icons.icon("mdi.content-copy"), QCoreApplication.translate("PropertiesDialog", 'Duplicate'),
                   window.duplicate_selection)
    menu.addAction(icons.icon("mdi.delete-outline"), QCoreApplication.translate("PropertiesDialog", 'Delete'),
                   window.scene.delete_selection)
    menu.addSeparator()
    menu.addAction(icons.icon("mdi.flip-horizontal"), QCoreApplication.translate("PropertiesDialog", 'Flip horizontal'),
                   lambda: window.scene.mirror_selection(True))
    menu.addAction(icons.icon("mdi.flip-vertical"), QCoreApplication.translate("PropertiesDialog", 'Flip vertical'),
                   lambda: window.scene.mirror_selection(False))
    menu.addAction(icons.icon("mdi.arrange-bring-to-front"), QCoreApplication.translate("PropertiesDialog", 'Bring to front'),
                   lambda: window.reorder_item(item, "front"))
    menu.addAction(icons.icon("mdi.arrange-bring-forward"), QCoreApplication.translate("PropertiesDialog", 'Bring forward'),
                   lambda: window.reorder_item(item, "forward"))
    menu.addAction(icons.icon("mdi.arrange-send-backward"), QCoreApplication.translate("PropertiesDialog", 'Send backward'),
                   lambda: window.reorder_item(item, "backward"))
    menu.addAction(icons.icon("mdi.arrange-send-to-back"), QCoreApplication.translate("PropertiesDialog", 'Send to back'),
                   lambda: window.reorder_item(item, "back"))
    menu.addSeparator()
    if isinstance(item, GroupItem):
        menu.addAction(icons.icon("mdi.ungroup"), QCoreApplication.translate("PropertiesDialog", 'Ungroup'),
                       window.scene.ungroup_selection)
    else:
        menu.addAction(icons.icon("mdi.group"), QCoreApplication.translate("PropertiesDialog", 'Group selection'),
                       window.scene.group_selection)
    if isinstance(item, _EXPLODABLE):
        menu.addAction(icons.icon("mdi.arrow-expand-all"),
                       QCoreApplication.translate("PropertiesDialog", 'Explode shape'), window.scene.explode_selection)
    return menu
...
def _solid_menu(menu, window, item):
    ...
    views = menu.addMenu(icons.icon("mdi.cube-scan"), QCoreApplication.translate("PropertiesDialog", 'View from'))
    for label, az, el in ((QCoreApplication.translate("PropertiesDialog", 'Isometric'), 45, 35.264), (QCoreApplication.translate("PropertiesDialog", 'Front'), 0, 0),
                          (QCoreApplication.translate("PropertiesDialog", 'Side'), 90, 0), (QCoreApplication.translate("PropertiesDialog", 'Top'), 0, 90),
                          (QCoreApplication.translate("PropertiesDialog", 'Default'), math.degrees(solids.DEFAULT_AZ),
    ...
    colours = menu.addMenu(icons.icon("mdi.palette-outline"), QCoreApplication.translate("PropertiesDialog", 'Solid colour'))
    for name, hex_color in solids.COLORS.items():
        colours.addAction(QCoreApplication.translate("PropertiesDialog", name.capitalize()), lambda c=hex_color:
